    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        /*try*/ {
            ...
            String senha = request.getParameter("senha_usuario");
                ...
                if(usuario.equals("natanael") && (senha.equals("123")))
                {
                    //response.getWriter().print("Login ok");
                    //request.getRequestDispatcher("calculo.jsp").forward(request, response);
                    //request.getRequestDispatcher("calculadora.jsp").forward(request, response);
                    request.getRequestDispatcher("indexNata.jsp").forward(request, response);
                    
                }else{
                     response.getWriter().print("Erro ao efetuar login!");
    }
        }
    }
